#!/usr/bin/env python
"""
baseline.py — SIDER baseline (SMILES only).
...
Example:
  python scripts/baseline/sider_baseline.py
"""
...
def build_user_prompt(smiles, task_name=None):
    task_name = task_name or "side_effect"

    if TASK_TYPE == "classification":
        return (
            "Given the SMILES string of a molecule, predict whether it is associated with the adverse effect category listed in the task field. "
            "Task/adverse effect category: {task_name}. "
        ...
        try:
            for prompt_type in PROMPT_TYPES:
                ...
                for idx, example in valid_examples:
                    ...
                    task_name = example.get("task", "side_effect")
