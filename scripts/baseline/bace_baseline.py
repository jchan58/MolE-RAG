"""
smiles.py — BACE baseline (SMILES only) across configured models.

Task: predict BACE-1 inhibition (Yes/No).
Output internally mapped to "1"/"0" for downstream evaluation.

Expected input CSV columns:
  dataset, task, task_type, metric, smiles, label, split

Example run:
  python scripts/baseline/bace_baseline.py
"""
...
MODEL_REGISTRY = {
    ...
    # LlaSMol expects tagged chemistry inputs such as:
    # <SMILES> ... </SMILES>
    # and often returns tagged outputs such as:
    # <BOOLEAN> Yes </BOOLEAN>
...
def get_model_short(model_name):
    """Return a filesystem-safe short name for output filenames."""
    return (
        model_name.split("/")[-1]
        .lower()
        .replace("-", "_")
...
def load_model(model_name):
    """Load model/tokenizer or initialize OpenAI client."""
    ...
    if cfg["format"] == "openai":
        ...
        if not api_key or api_key.startswith("sk-proj-REPLACE"):
            raise RuntimeError(
                ...
                "Put it in the .env at the repo root."
            )
    ...
    # ChemLLM/InternLM-style models can crash with KV cache under newer transformers.
...
def unload_model():
    """Free model/tokenizer and clear GPU cache."""
...
def _build_text(system_prompt, user_prompt, fmt):
    """Build the full input text for one prompt."""
    if fmt == "qwen":
        messages = [
    ...
    # LlaSMol official usage sends the chemistry query directly with tags:
    # <SMILES> ... </SMILES>
    # Do not wrap it in a chat template.
    if fmt == "llasmol":
        return user_prompt
...
def _manual_greedy_decode_hf(model_inputs, input_lengths, max_new_tokens):
    """
    ChemLLM fallback: manually decode without KV cache to avoid InternLM
    past_key_values crashes under newer transformers.
    """
    ...
    with torch.no_grad():
        for _ in range(max_new_tokens):
            ...
            if (
                ...
                and bool((next_token == _tokenizer.eos_token_id).all())
            ):
                break
...
def _run_inference_batch_hf(system_prompt, user_prompts, max_new_tokens=256):
    ...
    # ChemLLM-safe no-cache path.
    if fmt == "chemllm":
        return _manual_greedy_decode_hf(
...
def run_inference_batch(system_prompt, user_prompts, max_new_tokens=256):
    """Dispatch a batch of prompts to the appropriate backend."""
    fmt = _model_cfg["format"]

    if fmt == "openai":
        return _run_inference_openai_batch(
            system_prompt,
            user_prompts,
            max_tokens=max_new_tokens,
        )

    return _run_inference_batch_hf(
        system_prompt,
        user_prompts,
        max_new_tokens=max_new_tokens,
    )
...
# =========================
# Yes/No output parser
# =========================
def parse_binary_output(model_output):
    """Map a Yes/No response to '1'/'0'. Returns None if unparseable."""
    if not model_output:
        return None

    raw = str(model_output).strip()

    # Prefer LlaSMol-style tags before stripping characters.
    tag_match = re.search(
...
def build_user_prompt(smiles):
    if _model_cfg is not None and _model_cfg.get("format") == "llasmol":
        return (
            f"Can <SMILES> {smiles} </SMILES> inhibit BACE-1 / beta-secretase?\n"
            "Respond with <BOOLEAN> Yes </BOOLEAN> or <BOOLEAN> No </BOOLEAN>."
        )

    return (
        "Given the SMILES string of a molecule, predict whether it inhibits "
...
def main():
    ...
    valid_examples = [
        ...
        if ex.get(SMILES_COL, "").strip()
        and ex.get(LABEL_COL, None) not in (None, "")
    ]
    ...
    print(f"{DATASET_NAME}: {len(valid_examples)} valid test molecules ({skipped} skipped)")
        ...
        try:
            for prompt_type in PROMPT_TYPES:
                ...
                for idx, example in valid_examples:
                    ...
                    user_prompt = build_user_prompt(smiles)
                    batch_meta.append((idx, smiles, true_label, user_prompt))

                final_outputs = []

                for batch_start in range(0, len(batch_meta), BATCH_SIZE):
                    batch = batch_meta[batch_start: batch_start + BATCH_SIZE]
                    user_prompts = [b[3] for b in batch]
                    model_outputs = run_inference_batch(SYSTEM_PROMPT, user_prompts)

                    for (idx, smiles, true_label, _), model_output in zip(batch, model_outputs):
