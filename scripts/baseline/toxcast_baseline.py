#!/usr/bin/env python
"""
baseline.py — ToxCast baseline (SMILES only).
...
Example:
  python scripts/baseline/toxcast_baseline.py
"""
...
def build_user_prompt(smiles, task_name=None):
    task_name = task_name or "toxcast_assay"

    if TASK_TYPE == "classification":
        return (
            "Given the SMILES string of a molecule, predict whether it is active in the ToxCast assay listed in the task field. "
            "Task/assay: {task_name}. "
        ...
        try:
            for prompt_type in PROMPT_TYPES:
                ...
                for idx, example in valid_examples:
                    ...
                    task_name = example.get("task", "toxcast_assay")
