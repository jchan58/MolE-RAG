#!/usr/bin/env python
"""
baseline.py — HIV baseline (SMILES only).
...
Example:
  python scripts/baseline/hiv_baseline.py
"""
...
def build_user_prompt(smiles, task_name=None):
    task_name = task_name or "HIV_active"

    if TASK_TYPE == "classification":
        return (
            "Given the SMILES string of a molecule, predict whether it is active against HIV or inhibits HIV replication. "
            ""
        ...
        try:
            for prompt_type in PROMPT_TYPES:
                ...
                for idx, example in valid_examples:
                    ...
                    task_name = example.get("task", "HIV_active")
