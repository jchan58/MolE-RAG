#!/usr/bin/env python
"""
baseline.py — BBBP baseline (SMILES only).
...
def build_user_prompt(smiles, task_name=None):
    task_name = task_name or "p_np"

    if TASK_TYPE == "classification":
        return (
            "Given the SMILES string of a molecule, predict whether it has blood-brain barrier permeability. "
            ""
        ...
        try:
            for prompt_type in PROMPT_TYPES:
                ...
                for idx, example in valid_examples:
                    ...
                    task_name = example.get("task", "p_np")
